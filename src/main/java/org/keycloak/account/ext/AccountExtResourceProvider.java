...
public class AccountExtResourceProvider implements RealmResourceProvider {
    ...
    private Stream<ClientModel> getApplications(KeycloakSession session, RealmModel realm, UserModel user) {
        ...
        List<ClientModel> sortedClients = clientIds.stream()
                .map(id -> session.clients().getClientById(realm, id))
                .filter(bearerOnly.negate())
                .sorted(Comparator.comparing(ClientModel::getName)) // Sort by client name
                .collect(Collectors.toList());

        // Return a stream from the sorted list
