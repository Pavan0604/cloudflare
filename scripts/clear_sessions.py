...
def get_users():
    try:
        ...
        if token is None:
            return []
        url = f"https://id.clevertap.net/auth/admin/realms/CleverTap/users?max=1000&enabled=true"
        headers = {
...
def delete_inactive_user_sessions():
    try:
        ...
        for user in users:
            user_id = user["id"]
            print("Deleting User session for user: ", user["username"])
        ...
        print(f"Error in delete_inactive_user_sessions: {e}")
